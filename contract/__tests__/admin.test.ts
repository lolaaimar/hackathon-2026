...
describe("GovFund admin", () => {
  ...
  describe("addMember", () => {
    it("inserts the commitment and increments the member count", () => {
      expect(sim.getLedger().Mem_memberCount).toEqual(0n);
  ...
  describe("removeMember", () => {
    ...
    beforeEach(() => {
      sim2 = new GovFundSimulator({ admin: ADMIN, approvalsRequired: 1n });
      sim2.setActor(ADMIN);
      sim2.addMember(COMMIT_A);
      sim2.addMember(COMMIT_B);
    });
    ...
    it("blocks removal that would make approvals unreachable", () => {
      // removing the last one leaves 0 < 1 -> blocked
      sim2.removeMember(COMMIT_A);
      expect(() => sim2.removeMember(COMMIT_B)).toThrow(
        "Removal would make approvals unreachable",
      );
    });

    it("fails when there are no members", () => {
      const sim0 = new GovFundSimulator({
        admin: ADMIN,
        approvalsRequired: 1n,
      });
      sim0.setActor(ADMIN);
      expect(() => sim0.removeMember(COMMIT_A)).toThrow("No members");
    });
  });

  describe("setQuorumPercent", () => {
    it("updates the quorum percent", () => {
      sim.setQuorumPercent(60n);
      expect(sim.getLedger().quorumPercent).toEqual(60n);
    });

    it("rejects values outside 1..100", () => {
      expect(() => sim.setQuorumPercent(0n)).toThrow("Invalid quorum percent");
      expect(() => sim.setQuorumPercent(101n)).toThrow(
        "Invalid quorum percent",
      );
    });

    it("fails when called by a non-admin", () => {
      sim.setActor(OTHER);
      expect(() => sim.setQuorumPercent(60n)).toThrow("Not admin");
    });
  });

  describe("setApprovalsRequired", () => {
    it("updates the reviewer threshold", () => {
      const sim3 = new GovFundSimulator({
        admin: ADMIN,
        approvalsRequired: 1n,
      });
      sim3.setActor(ADMIN);
      sim3.addMember(COMMIT_A);

      sim3.setApprovalsRequired(1n);
      expect(sim3.getLedger().approvalsRequired).toEqual(1n);
    });

    it("rejects zero", () => {
      expect(() => sim.setApprovalsRequired(0n)).toThrow(
        "Invalid approval threshold",
      );
    });

    it("rejects a threshold larger than the member count", () => {
      expect(() => sim.setApprovalsRequired(3n)).toThrow(
        "Invalid approval threshold",
      );
    });
    ...
    it("fails when called by a non-admin", () => {
      sim.setActor(OTHER);
      expect(() => sim.setApprovalsRequired(1n)).toThrow("Not admin");
    });
  });
});
